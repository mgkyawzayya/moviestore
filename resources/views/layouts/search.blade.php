    ...
    <div class="h-screen flex overflow-hidden bg-gray-100" x-data="{ sidebarOpen: false }" @keydown.window.escape="sidebarOpen = false">
            ...
            <div class="fixed inset-0 flex z-40">
                <div x-show="sidebarOpen" x-transition:enter-start="-translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave-start="translate-x-0" x-transition:leave-end="-translate-x-full" class="flex-1 flex flex-col max-w-xs w-full pt-5 pb-4 bg-gray-800 transform ease-in-out duration-300 ">
                    ...
                    <div class="flex-shrink-0 flex items-center px-4">
                        <img class="rounded-lg h-36 w-auto" src="/img/logo/drfone.svg" alt="Workflow" />
                    </div>
        ...
        <div class="hidden md:flex md:flex-shrink-0">
            <div class="flex flex-col w-64">
                <div class="flex items-center h-64 flex-shrink-0 px-4 bg-gray-800">
                    <img class="rounded-lg h-56 w-auto " src="/img/logo/drfone.svg" alt="Workflow" />
                </div>
        ...
        <div class="flex flex-col w-0 flex-1 overflow-hidden">
            <div class="relative z-10 flex-shrink-0 flex h-16 bg-white shadow">
                ...
                <div class="flex-1 px-4 flex justify-between">
                    <div class="flex-1 flex">
                        <form action="/admin/search" method="POST" role="search" class="w-full flex md:ml-0">
                            {{ csrf_field() }}
                            ...
                            <div class="relative w-full text-gray-400 focus-within:text-gray-600">
                                ...
                                <input id="search" name="query" class="block w-full h-full pl-8 pr-3 py-2 rounded-md text-gray-900 placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 sm:text-sm" placeholder="Search Movies" />
                            </div>
                        </form>
                    </div>
            ...
            <main class="flex-1 relative z-0 overflow-y-auto py-0 focus:outline-none" tabindex="0" x-data x-init="$el.focus()">
                @yield('content')
            </main>
        </div>
    </div>
